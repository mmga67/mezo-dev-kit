---
name: mdk-tokens-application
description: Use installed MDK token APIs for balances, allowances and explicit approval planning in an application.
---
...
`pnpm exec mdk docs show api:tokens`. If uncached, use
`pnpm exec mdk docs fetch api:tokens --offline`. Search the
matching local corpus for only the knowledge needed by the task.

## Procedure

1. Identify the token, owner and protocol-selected spender before reading an allowance; token metadata alone cannot establish spender authority.
2. Use the SDK approval plan to distinguish sufficient allowance, reset and approval. Keep approval and the action consuming it separate; reread after a confirmed reset or approval.
3. Keep values in token base units. Verify approval post-state through the public API instead of treating a transaction hash as an allowance change.
