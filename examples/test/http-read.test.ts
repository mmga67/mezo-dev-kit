...
import { createCoreReadClient, createRpcTransport } from "@mezo-dev-kit/core";
...
describe("the example HTTP read boundary", () => {
  test("normalizes provider hash casing for coordinate comparisons", async () => {
    const { transport } = fixture(({ id }) => ({
      jsonrpc: "2.0",
      id,
      result: { number: "0x1", hash: `0x${"AB".repeat(32)}`, timestamp: "0x10" },
    }));
  ...
  test("composes public packages and checks the numbered call's canonical anchor", async () => {
        ...
        method === "eth_chainId"
          ...
          : method === "eth_blockNumber"
            ...
            : method === "eth_getBlockByNumber"
              ? { number: `0x${blockNumber.toString(16)}`, hash: blockHash, timestamp: "0x10" }
              : "0x1234",
    }));
    ...
    expect(calls.find((call) => call.method === "eth_call")?.params).toEqual([
      {
  ...
  test.for(["0x00", "0x1\n"])("preserves EVM quantity validation for %s", async (result) => {
    const { transport } = fixture(({ id }) => ({ jsonrpc: "2.0", id, result }));
    await expect(transport.getChainId()).rejects.toMatchObject({ code: "InvalidRpcQuantity" });
  });
  ...
  test("rejects odd-length call data returned by a provider", async () => {
    const { transport } = fixture(({ id, method }) => ({
      jsonrpc: "2.0",
      id,
      result:
        method === "eth_getBlockByNumber"
    ...
    ).rejects.toMatchObject({ code: "InvalidHexData" });
  });
  ...
  test("denies source mutation before any HTTP request", async () => {
    let calls = 0;
    ...
    await expect(request({ method: "eth_sendTransaction", params: [] })).rejects.toMatchObject({
      code: "MethodDenied",
    });
    await expect(request({ method: "anvil_setBalance", params: [] })).rejects.toMatchObject({
      code: "MethodDenied",
    });
    expect(calls).toBe(0);
  });
