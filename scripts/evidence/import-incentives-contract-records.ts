...
for (const definition of definitions) {
  ...
  const provenanceEvidence = {
    reproduction: {
      ...
      librariesSha256,
      buildProcedure:
        "Fetch the recorded explorer source bundle, generate an isolated Foundry project with scripts/evidence/reproduce-explorer-contract.ts, run forge build with the exact compiler/settings, then compare creation/runtime executables and ABI semantics with scripts/evidence/compare-explorer-build.ts.",
