...
const descriptiveLink = manifest.replace("[`manifest-changelog.md`]", "[Manifest changelog]");
assert.deepEqual(validateManifestVersion(descriptiveLink, log), {
  version: "2.1.0",
  released: "2026-08-23",
  entries: 2,
});
assert.throws(
  () =>
    validateManifestVersion(
      descriptiveLink.replace("(./manifest-changelog.md)", "(./missing.md)"),
      log,
    ),
  /must link its improvement log/,
);
assert.throws(
