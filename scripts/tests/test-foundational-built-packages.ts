...
runNode(
  ...
  [
    ...
    "  ['ContractRegistryError', 'createContractRegistry', 'getNativeBridgeCalldataAbi', 'getNativeTokenProfile', 'getTokenInterface', 'isContractId', 'listContractIds', 'resolveBasicPoolInterface', 'resolveContract', 'resolveEvent', 'resolveHistoricalContractEvidence', 'resolveOperation', 'resolveRoleInterface', 'resolveRuntimeIdentity', 'resolveVotingInterface', 'resolveVotingRewardInterface'],",
...
runNode(
  ...
  [
    ...
    "import { EvmValueError, createAbiCodec, parseRpcQuantity, toRpcQuantity, parseUnitsExact, formatUnitsExact, parseAddress, parseUserAddress, parseHash32 } from '@mezo-dev-kit/evm';",
    "import { getTokenInterface } from '@mezo-dev-kit/contracts';",
    "const codec = createAbiCodec();",
    "const approve = getTokenInterface().find(entry => entry.type === 'function' && entry.name === 'approve');",
    "const account = parseAddress('0x' + '11'.repeat(20));",
    "assert.deepEqual(codec.decodeCalldata(approve, codec.encodeFunction(approve, [account, 7n])), [account, 7n]);",
    "assert.throws(() => codec.decodeCalldata(approve, '0x00000000'), EvmValueError);",
    "assert.equal(typeof codec.decodeEventWithHashes, 'function');",
